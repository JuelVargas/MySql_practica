...
/*5. P.A. QUE MUESTRE LA LISTA DE HERRAMIENTAS QUE TENGAN SALDO EN EL ALMECEN, ORDENADA EN 
FORMA ALFABETICA POR DESCRIPCION.*/
DELIMITER //
	CREATE PROCEDURE PAC_EJERCICIO5()
    BEGIN
		SELECT H.DESCRIPCION AS 'HERRAMIENTA', A.CSALDO 'SALDO'
...
/*6. P.A. QUE MUESTRE EL COSTO TOTAL DE ENTREGAS PARA CADA HERRAMIENTA EL AÑO 2018. INCLUYENDO 
SOLAMENTE EN LA CONSULTA, LAS HERRAMIENTAS QUE TENGAN POR LO MENOS 2 ENTREGAS.
*/
DELIMITER //
	CREATE PROCEDURE PAC_EJERCICIO6()
    BEGIN
		SELECT H.DESCRIPCION AS 'HERRAMIETA', SUM(H.PRECIO * E.CANTIDAD) AS 'COSTO TOTAL'
        FROM HERRAMIENTA H INNER JOIN ENTREGA E 
        ON E.CODIGO = H.CODIGO
        WHERE YEAR(FECHA) = 2023 
        GROUP BY E.CODIGO HAVING COUNT(E.NE) >=2;
    END//
DELIMITER ;
CALL PAC_EJERCICIO6;

/*7. P.A. QUE MUESTRE EL COSTO MAXIMO PARA CADA HERRAMIENTA, TOMANDO EN CUENTA, LAS 
HERRAMIENTAS CUYA DESCRIPCION TERMINA EN UNA VOCAL.*/
DELIMITER //
CREATE PROCEDURE PAC_EJERCICIO7()
BEGIN
    SELECT H.DESCRIPCION AS 'HERRAMIENTA', MAX(H.PRECIO * E.CANTIDAD) AS 'COSTO MAXIMO'
    FROM HERRAMIENTA H
    INNER JOIN ENTREGA E ON E.CODIGO = H.CODIGO
    WHERE H.DESCRIPCION REGEXP '[AEIOU]'
    GROUP BY H.DESCRIPCION;
END//
DELIMITER ;
CALL PAC_EJERCICIO7;

/*10. P.A. QUE MUESTRE PARA CADA HERRAMMIENTA EL PORCENTAJE DE LA CANTIDAD DE SALIDA Y LA CANTIDAD 
DE SALDO EN EL ALMACEN, CON RESPECTO A LA CANTIDAD DE ENTRADA. INCLUIR LAS HERRAMIENTAS 
CUYO NOMBRE TIENE UNA * S, M, N, Z * EN LA TERCERA LETRA Y CUYA CANTIDAD DE ENTRADA ESTA 20% 
POR ENCIMA DE LA CANTIDAD MINIMA ENTREGADA GENERAL.*/
DELIMITER //
	CREATE PROCEDURE PAC_EJERCICIO10()
    BEGIN
		SELECT H.DESCRIPCION AS 'HERRAMIENTA', (A.CSALIDA*100/A.CENTRADA) AS '% CANTIDAD SALIDA',
        (A.CSALDO*100/A.CENTRADA) AS '% CANTIDAD SALDO'
        FROM HERRAMIENTA H INNER JOIN ALMACEN A
        ON A.CODIGO = H.CODIGO
        WHERE H.DESCRIPCION REGEXP '^..[SMNZ]' 
        AND A.CENTRADA > (SELECT MIN(CANTIDAD) FROM ENTREGA) * 1.20;
    END//
DELIMITER ;

CALL PAC_EJERCICIO10;
