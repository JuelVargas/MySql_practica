...
-- ----------------------------------------
/*8. P.A. QUE MUESTRE LA DESCRIPCION DE UNA HERRAMIENTA CUYO CODIGO SE INTRODUCE POR PARAMETRO 
DE ENTRADA Y CALCULE EL PORCENTAJE DE SALDO EN EL ALMACEN CON RESPECTO A LA CANTIDAD DE 
ENTRADA*/
DELIMITER //
	CREATE PROCEDURE PAI_EJE8(IN COD CHAR(6))
    BEGIN
		IF EXISTS(SELECT * FROM HERRAMIENTA WHERE CODIGO = COD) THEN
			SELECT H.DESCRIPCION AS 'HERRAMIETA', (A.CSALDO * 100/A.CENTRADA) AS '% SALDO ALMACEN'
    ...
    END//
DELIMITER ;
CALL PAI_EJE8('H-0001');


/*10. P.A. QUE DETERMINE EL NUMERO DE ENTREGAS REALIZADAS EL SEGUNDO SEMESTRE DEL AÑO 2019, 
CORRESPONDIENTE A HERRAMIENTAS CUYA DESCRIPCION COMIENZA CON UNA LETRA QUE ES 
INTRODUCIDA POR PARAMETRO DE ENTRADA.*/
DELIMITER //
	CREATE PROCEDURE PAI_EJE10(IN LTR CHAR(1))
    BEGIN
		IF EXISTS (SELECT * FROM HERRAMIENTA WHERE DESCRIPCION LIKE CONCAT(LTR,'%')) THEN
			SELECT H.DESCRIPCION AS 'HERRAMIETA', COUNT(E.NE) AS 'ENTREGAS REALIZADA'
            FROM HERRAMIENTA H INNER JOIN ENTREGA E
            ON E.CODIGO = H.CODIGO
            WHERE YEAR(E.FECHA)=2023 AND MONTH(E.FECHA) BETWEEN 1 AND 6
            AND H.DESCRIPCION LIKE CONCAT(LTR,'%')
            GROUP BY H.DESCRIPCION;
        ELSE SELECT 'NO EXISTE HERRAMIENTA CON LETRA INTRODUCIDA' AS MENSAJE; END IF;
    END//
DELIMITER ;
